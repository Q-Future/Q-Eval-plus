...
class VideoAlignmentTask(BaseTask):
    """
    Task for evaluating video alignment with text prompts.
    """
    
    def __init__(self, model_name: str = "gpt-4o", 
                 data_path: str = "./info/video_alignment_pairs_tests.json",
                 data_root: str = "/DATA/wfr/Q-eval/dataset/AGI-Eval/Q-Eval-100K/"):
        """
        Initialize the video alignment task.
        ...
        """
        super().__init__(model_name, "video_alignment", data_root)
        self.data_path = data_path
        self.api_manager = APIManager()
        
    def run_sbs_evaluation(self) -> Dict[str, Any]:
        """
        Run side-by-side evaluation for video alignment.
        
        Returns:
            Evaluation results
        """
        self.logger.info("Starting Video Alignment Side-by-Side evaluation...")
        ...
        for item in tqdm(json_data, desc="Processing items"):
            ...
            self.logger.info(f"video align sbs: {count}; answer: {pred1}")
            ...
            self.logger.info(f"video align sbs: {count}; answer: {pred2}")
        ...
        self.logger.info(f"Video Alignment SBS Acc: {acc}, Align: {align_ratio}")
        ...
        # Save results
        self.save_results(final_results, "video_alignment_sbs")
        
        return final_results
    
    def run_choice_evaluation(self) -> Dict[str, Any]:
        """
        Run choice-based evaluation for video alignment.
        
        Returns:
            Evaluation results
        """
        self.logger.info("Starting Video Alignment Choice evaluation...")
        ...
        for item in tqdm(json_data, desc="Processing items"):
            ...
            if score1 > score2:
                score_info = f"Video 1 has a higher text-to-video alignment score than Video 2."
            else:
                score_info = f"Video 2 has a higher text-to-video alignment score than Video 1."
            ...
            for idx, choice in enumerate(item[2]['choices']):
                ...
                if idx == 0:
                    ...
                    # f"{score_info}\n"
                    gpt_prompt = (
                        f"These are two videos (Each video is represented by 4 sampled frames: the first video<image_1><image_2><image_3><image_4> and the second video<image_5><image_6><image_7><image_8>) generated by different T2I Models from the same prompt. The prompt is: \"{prompt}\".\n"
                ...
                self.logger.info(f"video align choice: {count}; answer: {pred_text}")
        ...
        # Save results
        self.save_results(final_results, "video_alignment_choice")
        
        return final_results
    
    def run(self, evaluation_type: str = "both") -> Dict[str, Any]:
        """
        Run the video alignment task.
