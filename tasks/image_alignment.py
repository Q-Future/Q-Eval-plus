...
class ImageAlignmentTask(BaseTask):
    """
    Task for evaluating image alignment with text prompts.
    """
    
    def __init__(self, model_name: str = "gpt-4o", 
                 data_path: str = "./info/image_alignment_pairs_test.json",
                 data_root: str = "/DATA/wfr/Q-eval/dataset/AGI-Eval/Q-Eval-100K/"):
        """
        Initialize the image alignment task.
        ...
        """
        super().__init__(model_name, "image_alignment", data_root)
        self.data_path = data_path
        self.api_manager = APIManager()
        
    def run_sbs_evaluation(self) -> Dict[str, Any]:
        """
        Run side-by-side evaluation for image alignment.
        
        Returns:
            Evaluation results
        """
        self.logger.info("Starting Image Alignment Side-by-Side evaluation...")
        ...
        self.logger.info(f"Image Alignment SBS Acc: {acc}, Align: {align_ratio}")
        ...
        # Save results
        self.save_results(final_results, "image_alignment_sbs")
        
        return final_results
    
    def run_choice_evaluation(self) -> Dict[str, Any]:
        """
        Run choice-based evaluation for image alignment.
        
        Returns:
            Evaluation results
        """
        self.logger.info("Starting Image Alignment Choice evaluation...")
        ...
        for item in tqdm(json_data, desc="Processing items"):
            ...
            if score1 > score2:
                score_info = f"Image 0 has a higher text-to-image alignment score than Image 1."
            else:
                score_info = f"Image 1 has a higher text-to-image alignment score than Image 0."
            ...
            for idx, choice in enumerate(item[2]['choices']):
                ...
                if idx == 0:
                    ...
                    #f"{score_info}\n"
                    gpt_prompt = (
                        f"These are two images <image_0><image_1> generated by different T2I Models from the same prompt. The prompt is: \"{prompt}\"\n"
        ...
        # Save results
        self.save_results(final_results, "image_alignment_choice")
        
        return final_results
    
    def run(self, evaluation_type: str = "both") -> Dict[str, Any]:
        """
        Run the image alignment task.
